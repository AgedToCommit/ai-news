...
2. **Decide:**
   - **KEEP + ENRICH** if there's meaningful new info → update `summary` to incorporate it. If `why_it_matters_brief` is absent (legacy item predating this field) OR `why_it_matters` changes, write/update `why_it_matters_brief` using `agent/prompts/brief.md`. Append a revision entry: `{at: <now>, by: "enrichment", change: "<one-line description — mention brief if updated, e.g. 'updated why_it_matters and brief' or 'backfilled why_it_matters_brief'>"}`. Set `enriched_overnight: true`. Set `last_revision_by: "enrichment"`.
